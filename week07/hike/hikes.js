...
export default class Hikes {
  ...
  showHikeList() {
    ...
    const commentList = this.getAllComments('hike');
    commentList.forEach(comment => {
      this.parentElement.appendChild(renderCommentList(comment));
    });
  ...
  getAllComments(type){
    const commentList = localStorage.getItem(type);
    let displayCommentList='';
    if (commentList !== null) {
      displayCommentList = JSON.parse(commentList);
    }
      
    console.log(displayCommentList);
...
localStorage.setItem('hike', JSON.stringify(tempCommentList));